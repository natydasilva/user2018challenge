...
meteoro <- inner_join(stations, rain1, by = "Station_number") %>%
  ...
  filter(113.6594 <= lon & lon <= 153.61194)   #dim 12786716 rows, 22 cols

mete_summer <- meteoro %>%
  filter(Month %in% c(12, 1, 2)) %>%
  group_by(Year, lon, lat) %>%
  summarise(Rainfallm = mean(Rainfall)) #dim 32204 rows, 4 cols
...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      fluidRow(
        column(width = 6, withSpinner(plotOutput(outputId = 'plotrainsummer'))),
        column(width = 6, withSpinner(plotOutput(outputId = 'plotrainwinter')))
      ))
    ),
    ...
    #       'plant',
    #       'Plants',
    #       list(`Native` = c("Callitris","Hakea", "Daviesia", "Flindersia", "Livistona"),
   # `Introduced` = c("Brachychiton", "Triodia", "Ficus")),
  #selected = "Native"))
    #       
    #     )
    #   ),
...
>>>>>>> 71233c3da8fec63772da8bb9c37297b0eca4dbf1
...
server <- function(input, output, session) {
  ...
  plantSub <- function(genus) {
    tx <-
      plants_sub %>% filter(rank %in% c("species", "subspecies")) %>%
  ...
  datosDesc <- datos %>% group_by(state, year) %>%
    ...
    mutate(
      precipitationAnnual = mean(precipitationAnnual, na.rm = TRUE),
    ...
    pl_plant <- function(pl, y = "all", dat) {
      if (y == "all") {
        ...
        map +
          geom_point(
            data = plants,
            aes(x = longitudeOriginal, y = latitudeOriginal),
            colour = "orange"
          ) +
          labs(
            y = "Latitude",
            x = "Longitude",
            title = paste(input$plant, input$year)
          )
      } else{
        plants <-
          plants_sub %>% filter(grepl(pl, scientificName), year == y)
  ...
  output$plotrainsummer <- renderPlot({
    if (input$year == "all") {
      ...
      map + geom_point(
        ...
        size = I(4),
        alpha = 1 / 3
      ) + labs(title = paste("Summer rain", input$year)) +
  ...
  output$plotrainwinter <- renderPlot({
    if (input$year == "all") {
      ...
      map + geom_point(
        ...
        labs(colour = "Mean rain",
             title = paste("Winter rain", input$year)) +
        scale_colour_viridis()
    }
  })
