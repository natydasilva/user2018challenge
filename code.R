...
triodia <- occurrences(taxon = "triodia", qa = "none",
                        download_reason_id = "testing")$data %>% mutate(plant ="Triodia")


brachychiton <- occurrences(taxon = "brachychiton", qa = "none",
                       download_reason_id = "testing")$data %>% mutate(plant ="Brachychiton")


flindersia <- occurrences(taxon = "flindersia", qa = "none",
                            download_reason_id = "testing")$data %>% mutate(plant ="Flindersia")


livistona <- occurrences(taxon = "livistona", qa = "none",
                          download_reason_id = "testing")$data %>% mutate(plant ="Livistona")


callitris <- occurrences(taxon = "callitris", qa = "none",
                         download_reason_id = "testing")$data %>% mutate(plant ="Callitris")


daviesia <- occurrences(taxon = "daviesia", qa = "none",
                         download_reason_id = "testing")$data %>% mutate(plant ="Daviesia")

ficus <- occurrences(taxon = "ficus", qa = "none",
                        download_reason_id = "testing")$data %>% mutate(plant ="Ficus")


hakea <- occurrences(taxon = "hakea", qa = "none",
                     download_reason_id = "testing")$data %>% mutate(plant ="Hakea")
